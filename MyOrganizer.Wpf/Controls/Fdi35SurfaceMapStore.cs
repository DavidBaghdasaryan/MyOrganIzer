...
/// <summary>
/// Mesh-specific FDI 35 crown map generated from MandibularSecondPremolarTemplate
/// + left laterality. Runtime loads the packed JSON. Does not copy FDI 15/34 indices.
/// </summary>
internal static class Fdi35SurfaceMapStore
{
    public const string PackUri =
        "pack://application:,,,/MyOrganizer.Wpf;component/Assets/Teeth/FDI35SurfaceMap.json";
    ...
    public static ClinicalSurfaceMap Build(MeshGeometry3D crown) =>
        MandibularSecondPremolarTemplate.Generate(crown, ToothSide.Left);

    public static string GenerateDefault()
    {
        var obj35 = FindObj("FDI35_High.obj") ?? throw new FileNotFoundException("FDI35_High.obj not found.");
        var json35 = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(obj35)!, "..", "FDI35SurfaceMap.json"));
        ...
        var map = Build(parts35.Crown);
        DumpMeans(parts35.Crown, map);
        ToothSurfaceLayoutStats.Log("A", "35", "template-left", parts35.Crown, map.TriangleSurface);
        ToothSurfaceTopology.LogAnalyze("A", "35", "template-left", parts35.Crown, map.TriangleSurface);
        ToothSurfaceLayoutStats.LogRedHeight("A", "35", parts35.Crown, map.TriangleSurface);
        var own = ToothSurfaceTopology.ValidateOwnership(map.TriangleSurface);
        var red = ToothSurfaceLayoutStats.RedHeightOf(parts35.Crown, map.TriangleSurface);
        var layout = ToothSurfaceLayoutStats.Json("35", "template-left", parts35.Crown, map.TriangleSurface);
        LogLayout(layout, stats35, own, red, parts35.Crown);
        ...
        if (red.Mean > 0.40 || red.PctHigh > 15)
            throw new InvalidDataException(
                "FDI35 color 0 is not the cervical neck meanZ01=" + red.Mean.ToString("0.###", CultureInfo.InvariantCulture) +
                " pctHigh=" + red.PctHigh.ToString("0.0"));
        if (stats35.CrownMeanZ < stats35.RootMeanZ)
            throw new InvalidDataException("FDI35 crown/root Z inverted.");
        Save(map, json35);
        ...
        if (after.Map14 != frozen.Map14 || after.Map15 != frozen.Map15 ||
            ...
            throw new InvalidDataException("approved teeth were modified while generating FDI 35.");
        return json35;
    }

    private readonly record struct FrozenHashes(
        string Map14, string Map15, string Map24, string Map25, string Map34, string Map44,
        string Map16, string Map26, string Map36, string Map46,
        string Obj14, string Obj15, string Obj24, string Obj25, string Obj34, string Obj44,
    ...
    private static FrozenHashes LogFrozenHashes(string when)
    {
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi35-template\",\"hypothesisId\":\"C\",\"location\":\"Fdi35SurfaceMapStore.cs\",\"message\":\"frozen-hashes\",\"data\":{\"when\":\"" +
                       when +
                       "\",\"map15\":\"" + hashes.Map15 +
                       "\",\"map25\":\"" + hashes.Map25 +
                       "\",\"map34\":\"" + hashes.Map34 +
    ...
    private static void LogLayout(
        string layout, StlMeshStats stats35,
        (int Dup, int Unassigned) own, ToothSurfaceLayoutStats.RedHeight red, MeshGeometry3D crown)
    {
        ...
        try
        {
            ...
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi35-template\",\"hypothesisId\":\"A\",\"location\":\"Fdi35SurfaceMapStore.cs\",\"message\":\"laterality\",\"data\":{" +
                       "\"mirrored\":" + (stats35.Mirrored ? "true" : "false") +
                       ",\"yawDeg\":" + stats35.YawDeg.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dx\":" + stats35.Dx.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dy\":" + stats35.Dy.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dz\":" + stats35.Dz.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"crownMeanZ\":" + stats35.CrownMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"rootMeanZ\":" + stats35.RootMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"crownUp\":" + (stats35.CrownMeanZ > stats35.RootMeanZ ? "true" : "false") +
                       ",\"nTri\":" + nTri +
                       ",\"m35\":" + m.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"d35\":" + d.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"b35\":" + b.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"l35\":" + p.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"canonicalAxesOk\":" + (ok ? "true" : "false") +
                       ",\"copiedFirstPremolarTriangleIds\":false" +
                       ",\"dup\":" + own.Dup +
        ...
        if (!ok)
            throw new InvalidDataException(
                "FDI35 axes failed m=" + m + " d=" + d + " b=" + b + " l=" + p);
    }
    ...
    public static void Save(ClinicalSurfaceMap map, string path)
    {
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi35-template\",\"hypothesisId\":\"D\",\"location\":\"Fdi35SurfaceMapStore.cs\",\"message\":\"saved\",\"data\":{\"n\":" +
                       ...
                       ",\"mesh\":\"FDI35_High.obj\",\"copiedFirstPremolarTriangleIds\":false" +
                       "},\"timestamp\":" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() + "}\n";
            File.AppendAllText(@"c:\Users\david\source\repos\MyOrganIzer\debug-ee2893.log", line);
        }
        catch { }
    ...
    private static ClinicalSurfaceMap? Read(MeshGeometry3D crown, Stream stream)
    {
        ...
        if (dto is null || dto.TriangleCount != nTri || string.IsNullOrEmpty(dto.Labels) || dto.Labels.Length != nTri)
            return null;
        if (!string.Equals(dto.Mesh, "FDI35_High.obj", StringComparison.OrdinalIgnoreCase))
            return null;
