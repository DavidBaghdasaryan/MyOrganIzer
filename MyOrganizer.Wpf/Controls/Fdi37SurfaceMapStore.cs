...
/// <summary>
/// Mesh-specific FDI 37 crown map generated from MandibularSecondMolarTemplate
/// + left laterality. Runtime loads the packed JSON. Does not copy FDI 36 indices.
/// </summary>
internal static class Fdi37SurfaceMapStore
{
    public const string PackUri =
        "pack://application:,,,/MyOrganizer.Wpf;component/Assets/Teeth/FDI37SurfaceMap.json";
    ...
    public static ClinicalSurfaceMap Build(MeshGeometry3D crown) =>
        MandibularSecondMolarTemplate.Generate(crown, ToothSide.Left);

    public static string GenerateDefault()
    {
        var obj37 = FindObj("FDI37_High.obj") ?? throw new FileNotFoundException("FDI37_High.obj not found.");
        var json37 = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(obj37)!, "..", "FDI37SurfaceMap.json"));
        ...
        var map = Build(parts37.Crown);
        DumpMeans(parts37.Crown, map);
        ToothSurfaceLayoutStats.Log("A", "37", "template-left", parts37.Crown, map.TriangleSurface);
        ToothSurfaceTopology.LogAnalyze("A", "37", "template-left", parts37.Crown, map.TriangleSurface);
        ToothSurfaceLayoutStats.LogRedHeight("A", "37", parts37.Crown, map.TriangleSurface);
        var own = ToothSurfaceTopology.ValidateOwnership(map.TriangleSurface);
        var red = ToothSurfaceLayoutStats.RedHeightOf(parts37.Crown, map.TriangleSurface);
        var layout = ToothSurfaceLayoutStats.Json("37", "template-left", parts37.Crown, map.TriangleSurface);
        LogLayout(layout, stats37, own, red, parts37.Crown);
        if (stats37.RootClusters < 2)
            throw new InvalidDataException("FDI37 expected 2 apical roots, got " + stats37.RootClusters);
        ...
        if (red.Mean > 0.40 || red.PctHigh > 15)
            throw new InvalidDataException(
                "FDI37 color 0 is not the cervical neck meanZ01=" + red.Mean.ToString("0.###", CultureInfo.InvariantCulture) +
                " pctHigh=" + red.PctHigh.ToString("0.0"));
        if (stats37.CrownMeanZ < stats37.RootMeanZ)
            throw new InvalidDataException("FDI37 crown/root Z inverted.");
        Save(map, json37);
        var after = LogFrozenHashes("post");
        if (after.Map17 != frozen.Map17 || after.Map27 != frozen.Map27 ||
            ...
            throw new InvalidDataException("approved teeth were modified while generating FDI 37.");
        return json37;
    }
    ...
    private readonly record struct FrozenHashes(
        ...
        string Map16, string Map26, string Map36, string Map46, string Map17, string Map27,
        ...
        string Obj16, string Obj26, string Obj36, string Obj46, string Obj17, string Obj27);
    ...
    private static FrozenHashes LogFrozenHashes(string when)
    {
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi37-template\",\"hypothesisId\":\"C\",\"location\":\"Fdi37SurfaceMapStore.cs\",\"message\":\"frozen-hashes\",\"data\":{\"when\":\"" +
                       when +
                       "\",\"map17\":\"" + hashes.Map17 +
                       "\",\"map27\":\"" + hashes.Map27 +
                       "\",\"map36\":\"" + hashes.Map36 +
    ...
    private static void LogLayout(
        string layout, StlMeshStats stats37,
        (int Dup, int Unassigned) own, ToothSurfaceLayoutStats.RedHeight red, MeshGeometry3D crown)
    {
        ...
        try
        {
            ...
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi37-template\",\"hypothesisId\":\"A\",\"location\":\"Fdi37SurfaceMapStore.cs\",\"message\":\"laterality\",\"data\":{" +
                       "\"mirrored\":" + (stats37.Mirrored ? "true" : "false") +
                       ",\"yawDeg\":" + stats37.YawDeg.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dx\":" + stats37.Dx.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dy\":" + stats37.Dy.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dz\":" + stats37.Dz.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"rootClusters\":" + stats37.RootClusters +
                       ",\"crownMeanZ\":" + stats37.CrownMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"rootMeanZ\":" + stats37.RootMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"crownUp\":" + (stats37.CrownMeanZ > stats37.RootMeanZ ? "true" : "false") +
                       ",\"nTri\":" + nTri +
                       ",\"m37\":" + m.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"d37\":" + d.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"b37\":" + b.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"l37\":" + p.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"canonicalAxesOk\":" + (ok ? "true" : "false") +
                       ",\"copiedFdi36TriangleIds\":false" +
                       ",\"dup\":" + own.Dup +
        ...
        if (!ok)
            throw new InvalidDataException(
                "FDI37 axes failed m=" + m + " d=" + d + " b=" + b + " l=" + p);
    }
    ...
    public static void Save(ClinicalSurfaceMap map, string path)
    {
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi37-template\",\"hypothesisId\":\"D\",\"location\":\"Fdi37SurfaceMapStore.cs\",\"message\":\"saved\",\"data\":{\"n\":" +
                       ...
                       ",\"mesh\":\"FDI37_High.obj\",\"copiedFdi36TriangleIds\":false" +
                       "},\"timestamp\":" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() + "}\n";
            File.AppendAllText(@"c:\Users\david\source\repos\MyOrganIzer\debug-ee2893.log", line);
        }
        catch { }
    ...
    private static ClinicalSurfaceMap? Read(MeshGeometry3D crown, Stream stream)
    {
        ...
        if (dto is null || dto.TriangleCount != nTri || string.IsNullOrEmpty(dto.Labels) || dto.Labels.Length != nTri)
            return null;
        if (!string.Equals(dto.Mesh, "FDI37_High.obj", StringComparison.OrdinalIgnoreCase))
            return null;
