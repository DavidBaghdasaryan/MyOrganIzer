...
/// <summary>
/// Mesh-specific FDI 27 crown map generated from MaxillarySecondMolarTemplate
/// + left laterality. Runtime loads the packed JSON. Does not copy FDI 17 indices.
/// </summary>
internal static class Fdi27SurfaceMapStore
{
    public const string PackUri =
        "pack://application:,,,/MyOrganizer.Wpf;component/Assets/Teeth/FDI27SurfaceMap.json";
    ...
    public static ClinicalSurfaceMap Build(MeshGeometry3D crown) =>
        MaxillarySecondMolarTemplate.Generate(crown, ToothSide.Left);

    public static string GenerateDefault()
    {
        var obj27 = FindObj("FDI27_High.obj") ?? throw new FileNotFoundException("FDI27_High.obj not found.");
        var obj17 = FindObj("FDI17_High.obj") ?? throw new FileNotFoundException("FDI17_High.obj not found.");
        var json27 = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(obj27)!, "..", "FDI27SurfaceMap.json"));
        ...
        var frozen17 = ReadLabelsFromFile(
            Path.Combine(Path.GetDirectoryName(json27)!, "FDI17SurfaceMap.json"),
            parts17.Crown.TriangleIndices.Count / 3);
        if (frozen17 is null)
        ...
        using (var fs = File.OpenRead(obj27))
            parts27 = StlToothLoader.LoadAlignedParts(
        ...
        LogLaterality(layout17, layout27, stats17, stats27, own, red, parts27.Crown);
        if (stats27.RootClusters < 3)
            throw new InvalidDataException("FDI27 expected 3 apical roots, got " + stats27.RootClusters);
        ...
        if (red.Mean > 0.40 || red.PctHigh > 15)
            throw new InvalidDataException(
                "FDI27 color 0 is not the cervical neck meanZ01=" + red.Mean.ToString("0.###", CultureInfo.InvariantCulture) +
                " pctHigh=" + red.PctHigh.ToString("0.0"));
        if (stats27.CrownMeanZ < stats27.RootMeanZ)
            throw new InvalidDataException("FDI27 crown/root Z inverted.");
        Save(map, json27);
        var after = LogFrozenHashes("post");
        if (after.Map17 != frozen.Map17 || after.Obj17 != frozen.Obj17 ||
            ...
            throw new InvalidDataException("approved teeth were modified while generating FDI 27.");
        return json27;
    }
    ...
    private readonly record struct FrozenHashes(
        ...
        string Map16, string Map26, string Map36, string Map46, string Map17,
        ...
        string Obj16, string Obj26, string Obj36, string Obj46, string Obj17);
    ...
    private static void LogLaterality(
        string layout17, string layout27, StlMeshStats stats17, StlMeshStats stats27,
        (int Dup, int Unassigned) own, ToothSurfaceLayoutStats.RedHeight red, MeshGeometry3D crown)
    {
        ...
        var m17 = MeanX(layout17, "mesial");
        var d17 = MeanX(layout17, "distal");
        var b17 = MeanY(layout17, "buccal");
        var p17 = MeanY(layout17, "inner");
        var m27 = MeanX(layout27, "mesial");
        var d27 = MeanX(layout27, "distal");
        var b27 = MeanY(layout27, "buccal");
        var p27 = MeanY(layout27, "inner");
        var ok17 = m17 > 0 && d17 < 0 && b17 > 0 && p17 < 0;
        var ok27 = m27 < 0 && d27 > 0 && b27 > 0 && p27 < 0;
        var ok = ok17 && ok27;
        ...
        if (!ok)
            throw new InvalidDataException(
                "laterality axes failed m17=" + m17 + " d17=" + d17 + " m27=" + m27 + " d27=" + d27);
    }
    ...
    private static ClinicalSurfaceMap? Read(MeshGeometry3D crown, Stream stream)
    {
        ...
        if (dto is null || dto.TriangleCount != nTri || string.IsNullOrEmpty(dto.Labels) || dto.Labels.Length != nTri)
            return null;
        if (!string.Equals(dto.Mesh, "FDI27_High.obj", StringComparison.OrdinalIgnoreCase))
            return null;
        var labels = new ClinicalSurface[nTri];
