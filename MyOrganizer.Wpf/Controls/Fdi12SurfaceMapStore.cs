...
/// <summary>
/// Mesh-specific FDI 12 crown map generated from MaxillaryLateralIncisorTemplate
/// + right laterality. Runtime loads the packed JSON. Does not copy FDI 11 indices.
/// </summary>
internal static class Fdi12SurfaceMapStore
{
    public const string PackUri =
        "pack://application:,,,/MyOrganizer.Wpf;component/Assets/Teeth/FDI12SurfaceMap.json";
    ...
    public static ClinicalSurfaceMap Build(MeshGeometry3D crown) =>
        MaxillaryLateralIncisorTemplate.Generate(crown, ToothSide.Right);

    public static string GenerateDefault()
    {
        var obj12 = FindObj("FDI12_High.obj") ?? throw new FileNotFoundException("FDI12_High.obj not found.");
        var json12 = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(obj12)!, "..", "FDI12SurfaceMap.json"));
        ...
        var map = Build(parts12.Crown);
        DumpMeans(parts12.Crown, map);
        ToothSurfaceLayoutStats.Log("A", "12", "template-right", parts12.Crown, map.TriangleSurface);
        ToothSurfaceTopology.LogAnalyze("A", "12", "template-right", parts12.Crown, map.TriangleSurface);
        ToothSurfaceLayoutStats.LogRedHeight("A", "12", parts12.Crown, map.TriangleSurface);
        var own = ToothSurfaceTopology.ValidateOwnership(map.TriangleSurface);
        var red = ToothSurfaceLayoutStats.RedHeightOf(parts12.Crown, map.TriangleSurface);
        var layout = ToothSurfaceLayoutStats.Json("12", "template-right", parts12.Crown, map.TriangleSurface);
        LogLayout(layout, stats12, own, red, parts12.Crown);
        ...
        if (red.Mean > 0.40 || red.PctHigh > 15)
            throw new InvalidDataException(
                "FDI12 color 0 is not the cervical neck meanZ01=" + red.Mean.ToString("0.###", CultureInfo.InvariantCulture) +
                " pctHigh=" + red.PctHigh.ToString("0.0"));
        if (stats12.CrownMeanZ < stats12.RootMeanZ)
            throw new InvalidDataException("FDI12 crown/root Z inverted.");
        Save(map, json12);
        var after = LogFrozenHashes("post");
        if (after.Map11 != frozen.Map11 || after.Map21 != frozen.Map21 ||
            ...
            throw new InvalidDataException("approved teeth were modified while generating FDI 12.");
        return json12;
    }

    private readonly record struct FrozenHashes(
        string Map11, string Map21, string Map13, string Map23, string Map33, string Map43,
        ...
        string Obj11, string Obj21, string Obj13, string Obj23, string Obj33, string Obj43,
    ...
    private static FrozenHashes LogFrozenHashes(string when)
    {
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi12-template\",\"hypothesisId\":\"C\",\"location\":\"Fdi12SurfaceMapStore.cs\",\"message\":\"frozen-hashes\",\"data\":{\"when\":\"" +
                       when +
                       "\",\"map11\":\"" + hashes.Map11 +
                       "\",\"map21\":\"" + hashes.Map21 +
                       "\",\"map13\":\"" + hashes.Map13 +
    ...
    private static void LogLayout(
        string layout, StlMeshStats stats12,
        (int Dup, int Unassigned) own, ToothSurfaceLayoutStats.RedHeight red, MeshGeometry3D crown)
    {
        ...
        try
        {
            ...
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi12-template\",\"hypothesisId\":\"A\",\"location\":\"Fdi12SurfaceMapStore.cs\",\"message\":\"laterality\",\"data\":{" +
                       "\"mirrored\":" + (stats12.Mirrored ? "true" : "false") +
                       ",\"yawDeg\":" + stats12.YawDeg.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dx\":" + stats12.Dx.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dy\":" + stats12.Dy.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dz\":" + stats12.Dz.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"crownMeanZ\":" + stats12.CrownMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"rootMeanZ\":" + stats12.RootMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"crownUp\":" + (stats12.CrownMeanZ > stats12.RootMeanZ ? "true" : "false") +
                       ",\"nTri\":" + nTri +
                       ",\"m12\":" + m.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"d12\":" + d.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"b12\":" + b.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"p12\":" + p.ToString("0.###", CultureInfo.InvariantCulture) +
        ...
        if (!ok)
            throw new InvalidDataException(
                "FDI12 axes failed m=" + m + " d=" + d + " b=" + b + " p=" + p);
    }
    ...
    public static void Save(ClinicalSurfaceMap map, string path)
    {
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi12-template\",\"hypothesisId\":\"D\",\"location\":\"Fdi12SurfaceMapStore.cs\",\"message\":\"saved\",\"data\":{\"n\":" +
                       ...
                       ",\"mesh\":\"FDI12_High.obj\",\"copiedCentralIncisorTriangleIds\":false" +
                       "},\"timestamp\":" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() + "}\n";
            File.AppendAllText(@"c:\Users\david\source\repos\MyOrganIzer\debug-ee2893.log", line);
        }
        catch { }
    ...
    private static ClinicalSurfaceMap? Read(MeshGeometry3D crown, Stream stream)
    {
        ...
        if (dto is null || dto.TriangleCount != nTri || string.IsNullOrEmpty(dto.Labels) || dto.Labels.Length != nTri)
            return null;
        if (!string.Equals(dto.Mesh, "FDI12_High.obj", StringComparison.OrdinalIgnoreCase))
            return null;
