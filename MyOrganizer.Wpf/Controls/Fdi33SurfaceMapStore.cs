...
/// <summary>
/// Mesh-specific FDI 33 crown map generated from MandibularCanineTemplate
/// + left laterality. Runtime loads the packed JSON. Does not copy FDI 13 indices.
/// </summary>
internal static class Fdi33SurfaceMapStore
{
    public const string PackUri =
        "pack://application:,,,/MyOrganizer.Wpf;component/Assets/Teeth/FDI33SurfaceMap.json";
    ...
    public static ClinicalSurfaceMap Build(MeshGeometry3D crown) =>
        MandibularCanineTemplate.Generate(crown, ToothSide.Left);

    public static string GenerateDefault()
    {
        var obj33 = FindObj("FDI33_High.obj") ?? throw new FileNotFoundException("FDI33_High.obj not found.");
        var json33 = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(obj33)!, "..", "FDI33SurfaceMap.json"));
        ...
        LogLayout(layout, stats33, own, red, parts33.Crown);
        ...
        if (red.Mean > 0.40 || red.PctHigh > 15)
            throw new InvalidDataException(
                "FDI33 color 0 is not the cervical neck meanZ01=" + red.Mean.ToString("0.###", CultureInfo.InvariantCulture) +
                " pctHigh=" + red.PctHigh.ToString("0.0"));
        if (stats33.CrownMeanZ < stats33.RootMeanZ)
            throw new InvalidDataException("FDI33 crown/root Z inverted.");
        Save(map, json33);
        ...
        if (after.Map13 != frozen.Map13 || after.Map23 != frozen.Map23 ||
            ...
            throw new InvalidDataException("approved teeth were modified while generating FDI 33.");
        return json33;
    }

    private readonly record struct FrozenHashes(
        string Map13, string Map23, string Map14, string Map15, string Map24, string Map25,
        string Map34, string Map35, string Map44, string Map45,
        string Map16, string Map26, string Map36, string Map46,
        string Obj13, string Obj23, string Obj14, string Obj15, string Obj24, string Obj25,
    ...
    private static void LogLayout(
        string layout, StlMeshStats stats33,
    ...
    private static ClinicalSurfaceMap? Read(MeshGeometry3D crown, Stream stream)
    {
        ...
        if (dto is null || dto.TriangleCount != nTri || string.IsNullOrEmpty(dto.Labels) || dto.Labels.Length != nTri)
            return null;
        if (!string.Equals(dto.Mesh, "FDI33_High.obj", StringComparison.OrdinalIgnoreCase))
            return null;
