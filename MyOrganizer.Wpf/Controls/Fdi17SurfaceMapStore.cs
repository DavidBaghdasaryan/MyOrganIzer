...
/// <summary>
/// Mesh-specific FDI 17 crown map generated from MaxillarySecondMolarTemplate
/// + right laterality. Runtime loads the packed JSON. Does not copy FDI 16 indices.
/// </summary>
internal static class Fdi17SurfaceMapStore
{
    public const string PackUri =
        "pack://application:,,,/MyOrganizer.Wpf;component/Assets/Teeth/FDI17SurfaceMap.json";
    ...
    public static ClinicalSurfaceMap Build(MeshGeometry3D crown) =>
        MaxillarySecondMolarTemplate.Generate(crown, ToothSide.Right);

    public static string GenerateDefault()
    {
        var obj17 = FindObj("FDI17_High.obj") ?? throw new FileNotFoundException("FDI17_High.obj not found.");
        var json17 = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(obj17)!, "..", "FDI17SurfaceMap.json"));
        var frozen = LogFrozenHashes("pre");

        ToothMeshParts parts17;
        StlMeshStats stats17;
        using (var fs = File.OpenRead(obj17))
            parts17 = StlToothLoader.LoadAlignedParts(
                fs, out stats17, MaxillarySecondMolarTemplate.LoadOptions(ToothSide.Right));
        var map = Build(parts17.Crown);
        DumpMeans(parts17.Crown, map);
        ToothSurfaceLayoutStats.Log("A", "17", "template-right", parts17.Crown, map.TriangleSurface);
        ToothSurfaceTopology.LogAnalyze("A", "17", "template-right", parts17.Crown, map.TriangleSurface);
        ToothSurfaceLayoutStats.LogRedHeight("A", "17", parts17.Crown, map.TriangleSurface);
        var own = ToothSurfaceTopology.ValidateOwnership(map.TriangleSurface);
        var red = ToothSurfaceLayoutStats.RedHeightOf(parts17.Crown, map.TriangleSurface);
        var layout = ToothSurfaceLayoutStats.Json("17", "template-right", parts17.Crown, map.TriangleSurface);
        LogLayout(layout, stats17, own, red, parts17.Crown);
        if (stats17.RootClusters < 3)
            throw new InvalidDataException("FDI17 expected 3 apical roots, got " + stats17.RootClusters);
        ...
        if (red.Mean > 0.40 || red.PctHigh > 15)
            throw new InvalidDataException(
                "FDI17 color 0 is not the cervical neck meanZ01=" + red.Mean.ToString("0.###", CultureInfo.InvariantCulture) +
                " pctHigh=" + red.PctHigh.ToString("0.0"));
        if (stats17.CrownMeanZ < stats17.RootMeanZ)
            throw new InvalidDataException("FDI17 crown/root Z inverted.");
        Save(map, json17);
        var after = LogFrozenHashes("post");
        if (after.Map32 != frozen.Map32 || after.Map42 != frozen.Map42 ||
            ...
            throw new InvalidDataException("approved teeth were modified while generating FDI 17.");
        return json17;
    }
    ...
    private static FrozenHashes LogFrozenHashes(string when)
    {
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi17-template\",\"hypothesisId\":\"C\",\"location\":\"Fdi17SurfaceMapStore.cs\",\"message\":\"frozen-hashes\",\"data\":{\"when\":\"" +
                       when +
                       "\",\"map42\":\"" + hashes.Map42 +
                       "\",\"map32\":\"" + hashes.Map32 +
                       "\",\"map16\":\"" + hashes.Map16 +
    ...
    private static void LogLayout(
        string layout, StlMeshStats stats17,
        (int Dup, int Unassigned) own, ToothSurfaceLayoutStats.RedHeight red, MeshGeometry3D crown)
    {
        ...
        try
        {
            ...
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi17-template\",\"hypothesisId\":\"A\",\"location\":\"Fdi17SurfaceMapStore.cs\",\"message\":\"laterality\",\"data\":{" +
                       "\"mirrored\":" + (stats17.Mirrored ? "true" : "false") +
                       ",\"yawDeg\":" + stats17.YawDeg.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dx\":" + stats17.Dx.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dy\":" + stats17.Dy.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dz\":" + stats17.Dz.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"rootClusters\":" + stats17.RootClusters +
                       ",\"crownMeanZ\":" + stats17.CrownMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"rootMeanZ\":" + stats17.RootMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"crownUp\":" + (stats17.CrownMeanZ > stats17.RootMeanZ ? "true" : "false") +
                       ",\"nTri\":" + nTri +
                       ",\"m17\":" + m.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"d17\":" + d.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"b17\":" + b.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"p17\":" + p.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"canonicalAxesOk\":" + (ok ? "true" : "false") +
                       ",\"copiedFdi16TriangleIds\":false" +
                       ",\"dup\":" + own.Dup +
        ...
        if (!ok)
            throw new InvalidDataException(
                "FDI17 axes failed m=" + m + " d=" + d + " b=" + b + " p=" + p);
    }
    ...
    public static void Save(ClinicalSurfaceMap map, string path)
    {
        ...
        var dto = new Dto
        {
            Mesh = "FDI17_High.obj",
            TriangleCount = n,
            Source = MaxillarySecondMolarTemplate.PipelineSource,
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi17-template\",\"hypothesisId\":\"D\",\"location\":\"Fdi17SurfaceMapStore.cs\",\"message\":\"saved\",\"data\":{\"n\":" +
                       ...
                       ",\"mesh\":\"FDI17_High.obj\",\"copiedFdi16TriangleIds\":false" +
                       "},\"timestamp\":" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() + "}\n";
            File.AppendAllText(@"c:\Users\david\source\repos\MyOrganIzer\debug-ee2893.log", line);
        }
        catch { }
    ...
    private static ClinicalSurfaceMap? Read(MeshGeometry3D crown, Stream stream)
    {
        ...
        if (dto is null || dto.TriangleCount != nTri || string.IsNullOrEmpty(dto.Labels) || dto.Labels.Length != nTri)
            return null;
        if (!string.Equals(dto.Mesh, "FDI17_High.obj", StringComparison.OrdinalIgnoreCase))
            return null;
