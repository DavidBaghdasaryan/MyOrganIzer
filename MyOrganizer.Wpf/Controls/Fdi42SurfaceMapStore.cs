...
/// <summary>
/// Mesh-specific FDI 42 crown map generated from MandibularLateralIncisorTemplate
/// + right laterality. Runtime loads the packed JSON. Does not copy FDI 32 indices.
/// </summary>
internal static class Fdi42SurfaceMapStore
{
    public const string PackUri =
        "pack://application:,,,/MyOrganizer.Wpf;component/Assets/Teeth/FDI42SurfaceMap.json";
    ...
    public static ClinicalSurfaceMap Build(MeshGeometry3D crown) =>
        MandibularLateralIncisorTemplate.Generate(crown, ToothSide.Right);

    public static string GenerateDefault()
    {
        var obj42 = FindObj("FDI42_High.obj") ?? throw new FileNotFoundException("FDI42_High.obj not found.");
        var obj32 = FindObj("FDI32_High.obj") ?? throw new FileNotFoundException("FDI32_High.obj not found.");
        var json42 = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(obj42)!, "..", "FDI42SurfaceMap.json"));
        var frozen = LogFrozenHashes("pre");

        ToothMeshParts parts32;
        StlMeshStats stats32;
        using (var fs = File.OpenRead(obj32))
            parts32 = StlToothLoader.LoadAlignedParts(
                fs, out stats32, MandibularLateralIncisorTemplate.LoadOptions(ToothSide.Left));
        var frozen32 = ReadLabelsFromFile(
            Path.Combine(Path.GetDirectoryName(json42)!, "FDI32SurfaceMap.json"),
            parts32.Crown.TriangleIndices.Count / 3);
        if (frozen32 is null)
            throw new InvalidDataException("frozen FDI32SurfaceMap.json could not be read for laterality compare.");
        var layout32 = ToothSurfaceLayoutStats.Json("32", "frozen-readonly", parts32.Crown, frozen32);
        ToothSurfaceLayoutStats.Log("A", "32", "frozen-readonly", parts32.Crown, frozen32);
        ToothSurfaceLayoutStats.LogRedHeight("A", "32", parts32.Crown, frozen32);

        ToothMeshParts parts42;
        StlMeshStats stats42;
        using (var fs = File.OpenRead(obj42))
            parts42 = StlToothLoader.LoadAlignedParts(
                fs, out stats42, MandibularLateralIncisorTemplate.LoadOptions(ToothSide.Right));
        var map = Build(parts42.Crown);
        DumpMeans(parts42.Crown, map);
        ToothSurfaceLayoutStats.Log("A", "42", "template-right", parts42.Crown, map.TriangleSurface);
        ToothSurfaceTopology.LogAnalyze("A", "42", "template-right", parts42.Crown, map.TriangleSurface);
        ToothSurfaceLayoutStats.LogRedHeight("A", "42", parts42.Crown, map.TriangleSurface);
        var own = ToothSurfaceTopology.ValidateOwnership(map.TriangleSurface);
        var red = ToothSurfaceLayoutStats.RedHeightOf(parts42.Crown, map.TriangleSurface);
        var layout42 = ToothSurfaceLayoutStats.Json("42", "template-right", parts42.Crown, map.TriangleSurface);
        LogLaterality(layout32, layout42, stats32, stats42, own, red, parts42.Crown);
        ...
        if (red.Mean > 0.40 || red.PctHigh > 15)
            throw new InvalidDataException(
                "FDI42 color 0 is not the cervical neck meanZ01=" + red.Mean.ToString("0.###", CultureInfo.InvariantCulture) +
                " pctHigh=" + red.PctHigh.ToString("0.0"));
        if (stats42.CrownMeanZ < stats42.RootMeanZ)
            throw new InvalidDataException("FDI42 crown/root Z inverted.");
        Save(map, json42);
        var after = LogFrozenHashes("post");
        if (after.Map32 != frozen.Map32 || after.Obj32 != frozen.Obj32 ||
            ...
            throw new InvalidDataException("approved teeth were modified while generating FDI 42.");
        return json42;
    }

    private readonly record struct FrozenHashes(
        string Map11, string Map12, string Map21, string Map22, string Map31, string Map41, string Map32,
        ...
        string Obj11, string Obj12, string Obj21, string Obj22, string Obj31, string Obj41, string Obj32,
    ...
    private static FrozenHashes LogFrozenHashes(string when)
    {
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi42-template\",\"hypothesisId\":\"C\",\"location\":\"Fdi42SurfaceMapStore.cs\",\"message\":\"frozen-hashes\",\"data\":{\"when\":\"" +
                       when +
                       "\",\"map32\":\"" + hashes.Map32 +
                       "\",\"map31\":\"" + hashes.Map31 +
                       "\",\"map41\":\"" + hashes.Map41 +
    ...
    private static void LogLaterality(
        string layout32, string layout42, StlMeshStats stats32, StlMeshStats stats42,
        (int Dup, int Unassigned) own, ToothSurfaceLayoutStats.RedHeight red, MeshGeometry3D crown)
    {
        ...
        var m32 = MeanX(layout32, "mesial");
        var d32 = MeanX(layout32, "distal");
        var b32 = MeanY(layout32, "buccal");
        var l32 = MeanY(layout32, "inner");
        var m42 = MeanX(layout42, "mesial");
        var d42 = MeanX(layout42, "distal");
        var b42 = MeanY(layout42, "buccal");
        var l42 = MeanY(layout42, "inner");
        var ok32 = m32 > 0 && d32 < 0 && b32 > 0.25 && (b32 - l32) > 0.25;
        var ok42 = m42 < 0 && d42 > 0 && b42 > 0.25 && (b42 - l42) > 0.25;
        var ok = ok32 && ok42;
        var cancelled = Math.Abs(m42 - m32) < 0.05 && Math.Abs(d42 - d32) < 0.05;
        var cervical = red.Mean <= 0.40 && red.PctHigh <= 15;
        // #region agent log
        try
        {
            var nTri = crown.TriangleIndices.Count / 3;
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi42-template\",\"hypothesisId\":\"A\",\"location\":\"Fdi42SurfaceMapStore.cs\",\"message\":\"laterality\",\"data\":{" +
                       "\"mirrored32\":" + (stats32.Mirrored ? "true" : "false") +
                       ",\"mirrored42\":" + (stats42.Mirrored ? "true" : "false") +
                       ",\"yawDeg\":" + stats42.YawDeg.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dx\":" + stats42.Dx.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dy\":" + stats42.Dy.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dz\":" + stats42.Dz.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"crownMeanZ\":" + stats42.CrownMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"rootMeanZ\":" + stats42.RootMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"crownUp\":" + (stats42.CrownMeanZ > stats42.RootMeanZ ? "true" : "false") +
                       ",\"nTri\":" + nTri +
                       ",\"m32\":" + m32.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"d32\":" + d32.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"b32\":" + b32.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"l32\":" + l32.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"m42\":" + m42.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"d42\":" + d42.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"b42\":" + b42.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"l42\":" + l42.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"canonicalAxesOk\":" + (ok ? "true" : "false") +
                       ",\"lateralityCancelled\":" + (cancelled ? "true" : "false") +
                       ",\"copiedFdi32TriangleIds\":false" +
                       ",\"dup\":" + own.Dup +
        ...
        // #endregion
        if (cancelled)
            throw new InvalidDataException("FDI 42 laterality cancelled: mesial/distal match FDI 32.");
        if (!ok)
            throw new InvalidDataException(
                "laterality axes failed m32=" + m32 + " d32=" + d32 + " m42=" + m42 + " d42=" + d42);
    }
    ...
    public static void Save(ClinicalSurfaceMap map, string path)
    {
        ...
        var dto = new Dto
        {
            Mesh = "FDI42_High.obj",
            TriangleCount = n,
            Source = MandibularLateralIncisorTemplate.PipelineSource,
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi42-template\",\"hypothesisId\":\"D\",\"location\":\"Fdi42SurfaceMapStore.cs\",\"message\":\"saved\",\"data\":{\"n\":" +
                       ...
                       ",\"mesh\":\"FDI42_High.obj\",\"copiedFdi32TriangleIds\":false" +
                       "},\"timestamp\":" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() + "}\n";
            File.AppendAllText(@"c:\Users\david\source\repos\MyOrganIzer\debug-ee2893.log", line);
        }
        catch { }
    ...
    private static ClinicalSurfaceMap? Read(MeshGeometry3D crown, Stream stream)
    {
        ...
        if (dto is null || dto.TriangleCount != nTri || string.IsNullOrEmpty(dto.Labels) || dto.Labels.Length != nTri)
            return null;
        if (!string.Equals(dto.Mesh, "FDI42_High.obj", StringComparison.OrdinalIgnoreCase))
            return null;
        var labels = new ClinicalSurface[nTri];
