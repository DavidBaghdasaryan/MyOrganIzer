...
/// <summary>
/// Mesh-specific FDI 28 crown map generated from MaxillaryThirdMolarTemplate
/// + left laterality. Runtime loads the packed JSON. Does not copy FDI 18 indices.
/// </summary>
internal static class Fdi28SurfaceMapStore
{
    public const string PackUri =
        "pack://application:,,,/MyOrganizer.Wpf;component/Assets/Teeth/FDI28SurfaceMap.json";
    ...
    public static ClinicalSurfaceMap Build(MeshGeometry3D crown) =>
        MaxillaryThirdMolarTemplate.Generate(crown, ToothSide.Left);

    public static string GenerateDefault()
    {
        var obj28 = FindObj("FDI28_High.obj") ?? throw new FileNotFoundException("FDI28_High.obj not found.");
        var obj18 = FindObj("FDI18_High.obj") ?? throw new FileNotFoundException("FDI18_High.obj not found.");
        var json28 = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(obj28)!, "..", "FDI28SurfaceMap.json"));
        var frozen = LogFrozenHashes("pre");

        ToothMeshParts parts18;
        StlMeshStats stats18;
        using (var fs = File.OpenRead(obj18))
            parts18 = StlToothLoader.LoadAlignedParts(
                fs, out stats18, MaxillaryThirdMolarTemplate.LoadOptions(ToothSide.Right));
        var frozen18 = ReadLabelsFromFile(
            Path.Combine(Path.GetDirectoryName(json28)!, "FDI18SurfaceMap.json"),
            parts18.Crown.TriangleIndices.Count / 3);
        if (frozen18 is null)
            throw new InvalidDataException("frozen FDI18SurfaceMap.json could not be read for laterality compare.");
        var layout18 = ToothSurfaceLayoutStats.Json("18", "frozen-readonly", parts18.Crown, frozen18);
        ToothSurfaceLayoutStats.Log("A", "18", "frozen-readonly", parts18.Crown, frozen18);
        ToothSurfaceLayoutStats.LogRedHeight("A", "18", parts18.Crown, frozen18);

        ToothMeshParts parts28;
        StlMeshStats stats28;
        using (var fs = File.OpenRead(obj28))
            parts28 = StlToothLoader.LoadAlignedParts(
                fs, out stats28, MaxillaryThirdMolarTemplate.LoadOptions(ToothSide.Left));
        var map = Build(parts28.Crown);
        DumpMeans(parts28.Crown, map);
        ToothSurfaceLayoutStats.Log("A", "28", "template-left", parts28.Crown, map.TriangleSurface);
        ToothSurfaceTopology.LogAnalyze("A", "28", "template-left", parts28.Crown, map.TriangleSurface);
        ToothSurfaceLayoutStats.LogRedHeight("A", "28", parts28.Crown, map.TriangleSurface);
        var own = ToothSurfaceTopology.ValidateOwnership(map.TriangleSurface);
        var red = ToothSurfaceLayoutStats.RedHeightOf(parts28.Crown, map.TriangleSurface);
        var layout28 = ToothSurfaceLayoutStats.Json("28", "template-left", parts28.Crown, map.TriangleSurface);
        LogLaterality(layout18, layout28, stats18, stats28, own, red, parts28.Crown);
        ...
        if (red.Mean > 0.40 || red.PctHigh > 15)
            throw new InvalidDataException(
                "FDI28 color 0 is not the cervical neck meanZ01=" + red.Mean.ToString("0.###", CultureInfo.InvariantCulture) +
                " pctHigh=" + red.PctHigh.ToString("0.0"));
        if (stats28.CrownMeanZ < stats28.RootMeanZ)
            throw new InvalidDataException("FDI28 crown/root Z inverted.");
        Save(map, json28);
        var after = LogFrozenHashes("post");
        if (after.Map18 != frozen.Map18 || after.Obj18 != frozen.Obj18 ||
            after.Map47 != frozen.Map47 || after.Obj47 != frozen.Obj47 ||
            after.Map37 != frozen.Map37 || after.Obj37 != frozen.Obj37 ||
            ...
            throw new InvalidDataException("approved teeth were modified while generating FDI 28.");
        return json28;
    }
    ...
    private static FrozenHashes LogFrozenHashes(string when)
    {
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi28-template\",\"hypothesisId\":\"C\",\"location\":\"Fdi28SurfaceMapStore.cs\",\"message\":\"frozen-hashes\",\"data\":{\"when\":\"" +
                       when +
                       "\",\"map18\":\"" + hashes.Map18 +
                       "\",\"map47\":\"" + hashes.Map47 +
                       "\",\"map17\":\"" + hashes.Map17 +
    ...
    private static void LogLaterality(
        string layout18, string layout28, StlMeshStats stats18, StlMeshStats stats28,
        (int Dup, int Unassigned) own, ToothSurfaceLayoutStats.RedHeight red, MeshGeometry3D crown)
    {
        ...
        var m18 = MeanX(layout18, "mesial");
        var d18 = MeanX(layout18, "distal");
        var b18 = MeanY(layout18, "buccal");
        var p18 = MeanY(layout18, "inner");
        var m28 = MeanX(layout28, "mesial");
        var d28 = MeanX(layout28, "distal");
        var b28 = MeanY(layout28, "buccal");
        var p28 = MeanY(layout28, "inner");
        var ok18 = m18 > 0 && d18 < 0 && b18 > 0 && p18 < 0;
        var ok28 = m28 < 0 && d28 > 0 && b28 > 0 && p28 < 0;
        var ok = ok18 && ok28;
        var cancelled = Math.Abs(m28 - m18) < 0.05 && Math.Abs(d28 - d18) < 0.05;
        var cervical = red.Mean <= 0.40 && red.PctHigh <= 15;
        // #region agent log
        try
        {
            var nTri = crown.TriangleIndices.Count / 3;
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi28-template\",\"hypothesisId\":\"A\",\"location\":\"Fdi28SurfaceMapStore.cs\",\"message\":\"laterality\",\"data\":{" +
                       "\"mirrored18\":" + (stats18.Mirrored ? "true" : "false") +
                       ",\"mirrored28\":" + (stats28.Mirrored ? "true" : "false") +
                       ",\"yawDeg\":" + stats28.YawDeg.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dx\":" + stats28.Dx.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dy\":" + stats28.Dy.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dz\":" + stats28.Dz.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"rootClusters\":" + stats28.RootClusters +
                       ",\"crownMeanZ\":" + stats28.CrownMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"rootMeanZ\":" + stats28.RootMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"crownUp\":" + (stats28.CrownMeanZ > stats28.RootMeanZ ? "true" : "false") +
                       ",\"nTri\":" + nTri +
                       ",\"m18\":" + m18.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"d18\":" + d18.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"b18\":" + b18.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"p18\":" + p18.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"m28\":" + m28.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"d28\":" + d28.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"b28\":" + b28.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"p28\":" + p28.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"canonicalAxesOk\":" + (ok ? "true" : "false") +
                       ",\"lateralityCancelled\":" + (cancelled ? "true" : "false") +
                       ",\"copiedFdi18TriangleIds\":false" +
                       ",\"dup\":" + own.Dup +
        ...
        // #endregion
        if (cancelled)
            throw new InvalidDataException("FDI 28 laterality cancelled: mesial/distal match FDI 18.");
        if (!ok)
            throw new InvalidDataException(
                "laterality axes failed m18=" + m18 + " d18=" + d18 + " m28=" + m28 + " d28=" + d28);
    }
    ...
    public static void Save(ClinicalSurfaceMap map, string path)
    {
        ...
        var dto = new Dto
        {
            Mesh = "FDI28_High.obj",
            TriangleCount = n,
            Source = MaxillaryThirdMolarTemplate.PipelineSource,
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi28-template\",\"hypothesisId\":\"D\",\"location\":\"Fdi28SurfaceMapStore.cs\",\"message\":\"saved\",\"data\":{\"n\":" +
                       ...
                       ",\"mesh\":\"FDI28_High.obj\",\"copiedFdi18TriangleIds\":false" +
                       "},\"timestamp\":" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() + "}\n";
            File.AppendAllText(@"c:\Users\david\source\repos\MyOrganIzer\debug-ee2893.log", line);
        }
        catch { }
    ...
    private static ClinicalSurfaceMap? Read(MeshGeometry3D crown, Stream stream)
    {
        ...
        if (dto is null || dto.TriangleCount != nTri || string.IsNullOrEmpty(dto.Labels) || dto.Labels.Length != nTri)
            return null;
        if (!string.Equals(dto.Mesh, "FDI28_High.obj", StringComparison.OrdinalIgnoreCase))
            return null;
        var labels = new ClinicalSurface[nTri];
