...
/// <summary>
/// Mesh-specific FDI 22 crown map generated from MaxillaryLateralIncisorTemplate
/// + left laterality. Runtime loads the packed JSON. Does not copy FDI 12 indices.
/// </summary>
internal static class Fdi22SurfaceMapStore
{
    public const string PackUri =
        "pack://application:,,,/MyOrganizer.Wpf;component/Assets/Teeth/FDI22SurfaceMap.json";
    ...
    public static ClinicalSurfaceMap Build(MeshGeometry3D crown) =>
        MaxillaryLateralIncisorTemplate.Generate(crown, ToothSide.Left);

    public static string GenerateDefault()
    {
        var obj22 = FindObj("FDI22_High.obj") ?? throw new FileNotFoundException("FDI22_High.obj not found.");
        var obj12 = FindObj("FDI12_High.obj") ?? throw new FileNotFoundException("FDI12_High.obj not found.");
        var json22 = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(obj22)!, "..", "FDI22SurfaceMap.json"));
        var frozen = LogFrozenHashes("pre");

        ToothMeshParts parts12;
        StlMeshStats stats12;
        using (var fs = File.OpenRead(obj12))
            parts12 = StlToothLoader.LoadAlignedParts(
                fs, out stats12, MaxillaryLateralIncisorTemplate.LoadOptions(ToothSide.Right));
        var frozen12 = ReadLabelsFromFile(
            Path.Combine(Path.GetDirectoryName(json22)!, "FDI12SurfaceMap.json"),
            parts12.Crown.TriangleIndices.Count / 3);
        if (frozen12 is null)
            throw new InvalidDataException("frozen FDI12SurfaceMap.json could not be read for laterality compare.");
        var layout12 = ToothSurfaceLayoutStats.Json("12", "frozen-readonly", parts12.Crown, frozen12);
        ToothSurfaceLayoutStats.Log("A", "12", "frozen-readonly", parts12.Crown, frozen12);
        ToothSurfaceLayoutStats.LogRedHeight("A", "12", parts12.Crown, frozen12);

        ToothMeshParts parts22;
        StlMeshStats stats22;
        using (var fs = File.OpenRead(obj22))
            parts22 = StlToothLoader.LoadAlignedParts(
                fs, out stats22, MaxillaryLateralIncisorTemplate.LoadOptions(ToothSide.Left));
        var map = Build(parts22.Crown);
        DumpMeans(parts22.Crown, map);
        ToothSurfaceLayoutStats.Log("A", "22", "template-left", parts22.Crown, map.TriangleSurface);
        ToothSurfaceTopology.LogAnalyze("A", "22", "template-left", parts22.Crown, map.TriangleSurface);
        ToothSurfaceLayoutStats.LogRedHeight("A", "22", parts22.Crown, map.TriangleSurface);
        var own = ToothSurfaceTopology.ValidateOwnership(map.TriangleSurface);
        var red = ToothSurfaceLayoutStats.RedHeightOf(parts22.Crown, map.TriangleSurface);
        var layout22 = ToothSurfaceLayoutStats.Json("22", "template-left", parts22.Crown, map.TriangleSurface);
        LogLaterality(layout12, layout22, stats12, stats22, own, red, parts22.Crown);
        ...
        if (red.Mean > 0.40 || red.PctHigh > 15)
            throw new InvalidDataException(
                "FDI22 color 0 is not the cervical neck meanZ01=" + red.Mean.ToString("0.###", CultureInfo.InvariantCulture) +
                " pctHigh=" + red.PctHigh.ToString("0.0"));
        if (stats22.CrownMeanZ < stats22.RootMeanZ)
            throw new InvalidDataException("FDI22 crown/root Z inverted.");
        Save(map, json22);
        var after = LogFrozenHashes("post");
        if (after.Map12 != frozen.Map12 || after.Obj12 != frozen.Obj12 ||
            after.Map11 != frozen.Map11 || after.Map21 != frozen.Map21 ||
            ...
            after.Obj11 != frozen.Obj11 || after.Obj21 != frozen.Obj21 ||
            ...
            throw new InvalidDataException("approved teeth were modified while generating FDI 22.");
        return json22;
    }

    private readonly record struct FrozenHashes(
        string Map11, string Map12, string Map21, string Map13, string Map23, string Map33, string Map43,
        ...
        string Obj11, string Obj12, string Obj21, string Obj13, string Obj23, string Obj33, string Obj43,
    ...
    private static FrozenHashes LogFrozenHashes(string when)
    {
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi22-template\",\"hypothesisId\":\"C\",\"location\":\"Fdi22SurfaceMapStore.cs\",\"message\":\"frozen-hashes\",\"data\":{\"when\":\"" +
                       when +
                       "\",\"map12\":\"" + hashes.Map12 +
                       "\",\"map11\":\"" + hashes.Map11 +
                       "\",\"map21\":\"" + hashes.Map21 +
    ...
    private static void LogLaterality(
        string layout12, string layout22, StlMeshStats stats12, StlMeshStats stats22,
        (int Dup, int Unassigned) own, ToothSurfaceLayoutStats.RedHeight red, MeshGeometry3D crown)
    {
        ...
        var m12 = MeanX(layout12, "mesial");
        var d12 = MeanX(layout12, "distal");
        var b12 = MeanY(layout12, "buccal");
        var p12 = MeanY(layout12, "inner");
        var m22 = MeanX(layout22, "mesial");
        var d22 = MeanX(layout22, "distal");
        var b22 = MeanY(layout22, "buccal");
        var p22 = MeanY(layout22, "inner");
        var ok12 = m12 < 0 && d12 > 0 && b12 > 0 && p12 < 0;
        var ok22 = m22 > 0 && d22 < 0 && b22 > 0 && p22 < 0;
        var ok = ok12 && ok22;
        var cancelled = Math.Abs(m22 - m12) < 0.05 && Math.Abs(d22 - d12) < 0.05;
        var cervical = red.Mean <= 0.40 && red.PctHigh <= 15;
        // #region agent log
        try
        {
            var nTri = crown.TriangleIndices.Count / 3;
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi22-template\",\"hypothesisId\":\"A\",\"location\":\"Fdi22SurfaceMapStore.cs\",\"message\":\"laterality\",\"data\":{" +
                       "\"mirrored12\":" + (stats12.Mirrored ? "true" : "false") +
                       ",\"mirrored22\":" + (stats22.Mirrored ? "true" : "false") +
                       ",\"yawDeg\":" + stats22.YawDeg.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dx\":" + stats22.Dx.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dy\":" + stats22.Dy.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dz\":" + stats22.Dz.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"crownMeanZ\":" + stats22.CrownMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"rootMeanZ\":" + stats22.RootMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"crownUp\":" + (stats22.CrownMeanZ > stats22.RootMeanZ ? "true" : "false") +
                       ",\"nTri\":" + nTri +
                       ",\"m12\":" + m12.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"d12\":" + d12.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"b12\":" + b12.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"p12\":" + p12.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"m22\":" + m22.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"d22\":" + d22.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"b22\":" + b22.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"p22\":" + p22.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"canonicalAxesOk\":" + (ok ? "true" : "false") +
                       ",\"lateralityCancelled\":" + (cancelled ? "true" : "false") +
                       ",\"copiedFdi12TriangleIds\":false" +
                       ",\"dup\":" + own.Dup +
        ...
        // #endregion
        if (cancelled)
            throw new InvalidDataException("FDI 22 laterality cancelled: mesial/distal match FDI 12.");
        if (!ok)
            throw new InvalidDataException(
                "laterality axes failed m12=" + m12 + " d12=" + d12 + " m22=" + m22 + " d22=" + d22);
    }
    ...
    public static void Save(ClinicalSurfaceMap map, string path)
    {
        ...
        var dto = new Dto
        {
            Mesh = "FDI22_High.obj",
            TriangleCount = n,
            Source = MaxillaryLateralIncisorTemplate.PipelineSource,
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi22-template\",\"hypothesisId\":\"D\",\"location\":\"Fdi22SurfaceMapStore.cs\",\"message\":\"saved\",\"data\":{\"n\":" +
                       ...
                       ",\"mesh\":\"FDI22_High.obj\",\"copiedFdi12TriangleIds\":false" +
                       "},\"timestamp\":" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() + "}\n";
            File.AppendAllText(@"c:\Users\david\source\repos\MyOrganIzer\debug-ee2893.log", line);
        }
        catch { }
    ...
    private static ClinicalSurfaceMap? Read(MeshGeometry3D crown, Stream stream)
    {
        ...
        if (dto is null || dto.TriangleCount != nTri || string.IsNullOrEmpty(dto.Labels) || dto.Labels.Length != nTri)
            return null;
        if (!string.Equals(dto.Mesh, "FDI22_High.obj", StringComparison.OrdinalIgnoreCase))
            return null;
        var labels = new ClinicalSurface[nTri];
