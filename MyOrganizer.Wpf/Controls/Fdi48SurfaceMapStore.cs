...
/// <summary>
/// Mesh-specific FDI 48 crown map generated from MandibularThirdMolarTemplate
/// + right laterality. Runtime loads the packed JSON. Does not copy FDI 38 indices.
/// </summary>
internal static class Fdi48SurfaceMapStore
{
    public const string PackUri =
        "pack://application:,,,/MyOrganizer.Wpf;component/Assets/Teeth/FDI48SurfaceMap.json";
    ...
    public static ClinicalSurfaceMap Build(MeshGeometry3D crown) =>
        MandibularThirdMolarTemplate.Generate(crown, ToothSide.Right);

    public static string GenerateDefault()
    {
        var obj48 = FindObj("FDI48_High.obj") ?? throw new FileNotFoundException("FDI48_High.obj not found.");
        var obj38 = FindObj("FDI38_High.obj") ?? throw new FileNotFoundException("FDI38_High.obj not found.");
        var json48 = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(obj48)!, "..", "FDI48SurfaceMap.json"));
        ...
        var frozen38 = ReadLabelsFromFile(
            Path.Combine(Path.GetDirectoryName(json48)!, "FDI38SurfaceMap.json"),
            parts38.Crown.TriangleIndices.Count / 3);
        if (frozen38 is null)
            throw new InvalidDataException("frozen FDI38SurfaceMap.json could not be read for laterality compare.");
        var layout38 = ToothSurfaceLayoutStats.Json("38", "frozen-readonly", parts38.Crown, frozen38);
        ToothSurfaceLayoutStats.Log("A", "38", "frozen-readonly", parts38.Crown, frozen38);
        ToothSurfaceLayoutStats.LogRedHeight("A", "38", parts38.Crown, frozen38);

        ToothMeshParts parts48;
        StlMeshStats stats48;
        using (var fs = File.OpenRead(obj48))
            parts48 = StlToothLoader.LoadAlignedParts(
                fs, out stats48, MandibularThirdMolarTemplate.LoadOptions(ToothSide.Right));
        var map = Build(parts48.Crown);
        DumpMeans(parts48.Crown, map);
        ToothSurfaceLayoutStats.Log("A", "48", "template-right", parts48.Crown, map.TriangleSurface);
        ToothSurfaceTopology.LogAnalyze("A", "48", "template-right", parts48.Crown, map.TriangleSurface);
        ToothSurfaceLayoutStats.LogRedHeight("A", "48", parts48.Crown, map.TriangleSurface);
        var own = ToothSurfaceTopology.ValidateOwnership(map.TriangleSurface);
        var red = ToothSurfaceLayoutStats.RedHeightOf(parts48.Crown, map.TriangleSurface);
        var layout48 = ToothSurfaceLayoutStats.Json("48", "template-right", parts48.Crown, map.TriangleSurface);
        LogLaterality(layout38, layout48, stats38, stats48, own, red, parts48.Crown);
        if (stats48.RootClusters < 1)
            throw new InvalidDataException("FDI48 expected at least 1 apical root, got " + stats48.RootClusters);
        ...
        if (red.Mean > 0.40 || red.PctHigh > 15)
            throw new InvalidDataException(
                "FDI48 color 0 is not the cervical neck meanZ01=" + red.Mean.ToString("0.###", CultureInfo.InvariantCulture) +
                " pctHigh=" + red.PctHigh.ToString("0.0"));
        if (stats48.CrownMeanZ < stats48.RootMeanZ)
            throw new InvalidDataException("FDI48 crown/root Z inverted.");
        Save(map, json48);
        var after = LogFrozenHashes("post");
        if (after.Map38 != frozen.Map38 || after.Obj38 != frozen.Obj38 ||
            after.Map18 != frozen.Map18 || after.Map28 != frozen.Map28 ||
            ...
            throw new InvalidDataException("approved teeth were modified while generating FDI 48.");
        return json48;
    }
    ...
    private readonly record struct FrozenHashes(
        ...
        string Map18, string Map28, string Map38,
        ...
        string Obj18, string Obj28, string Obj38);
    ...
    private static FrozenHashes LogFrozenHashes(string when)
    {
        ...
        var hashes = new FrozenHashes(
            ...
            HashOf("Assets/Teeth/FDI38SurfaceMap.json"),
            ...
            HashOf("Assets/Teeth/Source/FDI38_High.obj"));
        // #region agent log
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi48-template\",\"hypothesisId\":\"C\",\"location\":\"Fdi48SurfaceMapStore.cs\",\"message\":\"frozen-hashes\",\"data\":{\"when\":\"" +
                       when +
                       "\",\"map38\":\"" + hashes.Map38 +
                       "\",\"map18\":\"" + hashes.Map18 +
                       "\",\"map28\":\"" + hashes.Map28 +
    ...
    private static void LogLaterality(
        string layout38, string layout48, StlMeshStats stats38, StlMeshStats stats48,
        (int Dup, int Unassigned) own, ToothSurfaceLayoutStats.RedHeight red, MeshGeometry3D crown)
    {
        ...
        var m38 = MeanX(layout38, "mesial");
        var d38 = MeanX(layout38, "distal");
        var b38 = MeanY(layout38, "buccal");
        var l38 = MeanY(layout38, "inner");
        var m48 = MeanX(layout48, "mesial");
        var d48 = MeanX(layout48, "distal");
        var b48 = MeanY(layout48, "buccal");
        var l48 = MeanY(layout48, "inner");
        var ok38 = m38 > 0 && d38 < 0 && b38 > 0 && l38 < 0;
        var ok48 = m48 < 0 && d48 > 0 && b48 > 0 && l48 < 0;
        var ok = ok38 && ok48;
        var cancelled = Math.Abs(m48 - m38) < 0.05 && Math.Abs(d48 - d38) < 0.05;
        var cervical = red.Mean <= 0.40 && red.PctHigh <= 15;
        // #region agent log
        try
        {
            var nTri = crown.TriangleIndices.Count / 3;
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi48-template\",\"hypothesisId\":\"A\",\"location\":\"Fdi48SurfaceMapStore.cs\",\"message\":\"laterality\",\"data\":{" +
                       "\"mirrored38\":" + (stats38.Mirrored ? "true" : "false") +
                       ",\"mirrored48\":" + (stats48.Mirrored ? "true" : "false") +
                       ",\"yawDeg\":" + stats48.YawDeg.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dx\":" + stats48.Dx.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dy\":" + stats48.Dy.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"dz\":" + stats48.Dz.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"rootClusters\":" + stats48.RootClusters +
                       ",\"crownMeanZ\":" + stats48.CrownMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"rootMeanZ\":" + stats48.RootMeanZ.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"crownUp\":" + (stats48.CrownMeanZ > stats48.RootMeanZ ? "true" : "false") +
                       ",\"nTri\":" + nTri +
                       ",\"m38\":" + m38.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"d38\":" + d38.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"b38\":" + b38.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"l38\":" + l38.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"m48\":" + m48.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"d48\":" + d48.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"b48\":" + b48.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"l48\":" + l48.ToString("0.###", CultureInfo.InvariantCulture) +
                       ",\"canonicalAxesOk\":" + (ok ? "true" : "false") +
                       ",\"lateralityCancelled\":" + (cancelled ? "true" : "false") +
                       ",\"copiedFdi38TriangleIds\":false" +
                       ",\"dup\":" + own.Dup +
        ...
        // #endregion
        if (cancelled)
            throw new InvalidDataException("FDI 48 laterality cancelled: mesial/distal match FDI 38.");
        if (!ok)
            throw new InvalidDataException(
                "laterality axes failed m38=" + m38 + " d38=" + d38 + " m48=" + m48 + " d48=" + d48);
    }
    ...
    public static void Save(ClinicalSurfaceMap map, string path)
    {
        ...
        try
        {
            var line = "{\"sessionId\":\"ee2893\",\"runId\":\"fdi48-template\",\"hypothesisId\":\"D\",\"location\":\"Fdi48SurfaceMapStore.cs\",\"message\":\"saved\",\"data\":{\"n\":" +
                       ...
                       ",\"mesh\":\"FDI48_High.obj\",\"copiedFdi38TriangleIds\":false" +
                       "},\"timestamp\":" + DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() + "}\n";
            File.AppendAllText(@"c:\Users\david\source\repos\MyOrganIzer\debug-ee2893.log", line);
        }
        catch { }
    ...
    private static ClinicalSurfaceMap? Read(MeshGeometry3D crown, Stream stream)
    {
        ...
        if (dto is null || dto.TriangleCount != nTri || string.IsNullOrEmpty(dto.Labels) || dto.Labels.Length != nTri)
            return null;
        if (!string.Equals(dto.Mesh, "FDI48_High.obj", StringComparison.OrdinalIgnoreCase))
            return null;
        var labels = new ClinicalSurface[nTri];
